...
class PayrollApp(QMainWindow):
    ...
    def calculate_payroll(self):
        try:
            ...
            bhos = round(bhos,2)
            ryada = round(ryada,2)
            eshraf = round(eshraf,2)
            maktabia = round(maktabia,2)
            tatwer = round(tatwer,2)
            gawda = round(gawda,2)
            diff_gawda = round(diff_gawda,2)
            hafz = round(hafz,2)
            badl = round(badl,2)
    ...
    def export_view_to_pdf(self, table):
        ...
        try:
            ...
            for row in selected_rows:
                ...
                num_item = table.item(row, name_column_index)
                name_item = table.item(row, name_column_index+1)
                
                if name_item is not None:
                    employee_num = num_item.text()
                ...
                subtitle_text = arabic_reshaper.reshape(f"بيانات الموظف: {employee_name} رقم {employee_num}")
    ...
    def export_to_pdf(self):
        ...
        try:
            ...
            font_path = os.path.abspath("Amiri-Regular.ttf")  # Ensure the font file is in the same directory
            
            if not os.path.exists(font_path):
                QMessageBox.critical(self, "خطأ", "لم يتم العثور على خط Arabic! تأكد من وضع ملف Amiri-Regular.ttf في مجلد المشروع.")
                return
            ...
            title_style = ParagraphStyle(
                'Title',
                parent=styles['Heading1'],
                fontName='ArabicFont',  # ✅ Ensure we use our registered Arabic font
                fontSize=18,
                alignment=1,  # Center alignment
                spaceAfter=12
            )
            ...
            # ✅ Fix Arabic Date Issue
            current_date = datetime.now().strftime("%Y-%m-%d")
            date_label = arabic_reshaper.reshape("تاريخ الإصدار")
            date_bidi_label = get_display(date_label)
            date_bidi_value = get_display(current_date)  # Numbers don't need reshaping
    
            date_text = Paragraph(f"{date_bidi_label}: {date_bidi_value}", normal_style)
            ...
            style = TableStyle([
                ...
                ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
                ('FONTNAME', (0, 0), (-1, -1), 'ArabicFont'),  # ✅ Ensure Arabic font is used
                ('FONTSIZE', (0, 0), (-1, -1), 12),
            ...
            for i in range(len(data)):
                if i % 2 == 0:
                    style.add('BACKGROUND', (1, i), (1, i), colors.whitesmoke)
