...
class Backend:
    ...
    def ensure_patients_folder(self):
        """Create the 'patients' folder beside the prostate_modular directory if it does not exist."""
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        folder_path = os.path.join(base_dir, "patients")
    ...
    def create_patient_subfolder(self, patient_name):
        ...
        date_folder = date_folder.split("\\")[5:]  # Get the last part of the path (the date folder name)
        date_folder = os.path.join(*date_folder)  # Join the parts back to a string
