...
class FourthLogic:
    ...
    def __init__(self, gui):
        ...
        self.bottle_count = 21  # Total number of bottles (11-21)
        # Each bottle will have its own check_box list for tracking checked Gleason options
        self.check_boxes = {i: [] for i in range(11, self.bottle_count + 1)}
        ...
        for gleason_idx in range(1, 21):
            ...
            if cb and cb.get():
                ...
                if text not in self.check_boxes[bottle_num]:
                    ...
                    self.gui.entries_4[f"ca_prostate_{bottle_num}"] = int(value)
    ...
    def gleason_event(self, event, bottle_num):
        ...
        self.gui.entries_4[f"ca_prostate_{bottle_num}"] = value
        for gleason_idx in range(1, 21):
    ...
    def bind_events(self):
        ...
        """
        for bottle_num in range(11, self.bottle_count + 1):
            option_risk = getattr(self.gui, f"option_risk{bottle_num}", None)
            if option_risk:
    ...
    def reset_bottle(self, bottle_num):
        ...
        for key in list(self.gui.entries_4.keys()):
            if key.endswith(f"_{bottle_num}"):
                self.gui.entries_4[key] = None
    ...
    def update_gleason_selection(self, bottle_num, idx, checked):
        ...
        if checked:
            ...
            if value is not None and value.strip() != "":
                if old_key:
                    self.gui.entries_4[old_key] = value
        else:
            if old_key:
                self.gui.entries_4[old_key] = None